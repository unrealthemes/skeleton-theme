...
class UT_Breadcrumbs {
	...
	static $args = array(
		...
		'markup'          => 'schema.org', // 'markup' - микроразметка. Может быть: 'rdf.data-vocabulary.org', 'schema.org', '' - без микроразметки
										   ...
										   // array( 'wrappatt'=>'<div class="ut_breadcrumbs">%s</div>', 'linkpatt'=>'<a href="%s">%s</a>', 'sep_after'=>'', )
	...
	function get_crumbs( $sep, $l10n, $args ){
		...
		$loc = (object) array_merge( apply_filters('ut_breadcrumbs_default_loc', self::$l10n ), $l10n );
		$arg = (object) array_merge( apply_filters('ut_breadcrumbs_default_args', self::$args ), $args );
		...
		if(1){
			...
			elseif( $mark === 'rdf.data-vocabulary.org' ) $mark = array(
				'wrappatt'   => '<ul prefix="v: http://rdf.data-vocabulary.org/#">%s</ul>',
			...
			elseif( $mark === 'schema.org' ) $mark = array(
				'wrappatt'   => '<ul itemscope itemtype="http://schema.org/BreadcrumbList">%s</ul>',
		...
		// Таксы, плоские записи и вложения
			...
			if( $term && isset($term->term_id) ){
				$term = apply_filters('ut_breadcrumbs_term', $term );
		...
		$home_after = apply_filters('ut_breadcrumbs_home_after', '', $linkpatt, $sep, $ptype );
		...
		$out = apply_filters('ut_breadcrumbs_pre_out', $out, $sep, $loc, $arg );

		$out = sprintf( $wrappatt, $before_out . $out );

		return apply_filters('ut_breadcrumbs', $out, $sep, $loc, $arg );
	}
 ...
 * 2.0 - ADD: в фильтр 'ut_breadcrumbs_home_after' добавлен четвертый аргумент $ptype
 * 1.9 - ADD: фильтр 'ut_breadcrumbs_default_loc' для изменения локализации по умолчанию
